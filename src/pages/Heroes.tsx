...
export default function Heroes() {
  ...
  const fetchHeroes = async () => {
    try {
      // Fetch all resolved FOUND items
      ...
      if (data) {
        // Group and count by reporter_name
        const counts: Record<string, number> = {};
        data.forEach((item) => {
          let name = item.reporter_name?.trim();
          if (!name) name = 'Anonim';
          
          // Case insensitive grouping (capitalize first letter for display)
          const lowerName = name.toLowerCase();
          const displayKey = counts[lowerName] ? Object.keys(counts).find(k => k.toLowerCase() === lowerName) || name : name;
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden relative">
          <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500"></div>
